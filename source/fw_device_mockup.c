...
int run_device(state_machine_event_queue_t event_queue) {
  ...
  while (g_enable_run_device) {
    ...
    FW_LOG_INFO("Triggering random event: %s",
                ENUM_STRING_FROM_TYPE(random_event, "main_state_machine_events"));
    ...
    if (g_events_pushed_count > PUSHED_EVENTS_LIMIT) {
      ...
      FW_LOG_INFO("Reached the limit (%d) of pushed events. Stopping the device...",
                  PUSHED_EVENTS_LIMIT);
    }
  }
  return 0;
...
void set_run_device(bool enable) { g_enable_run_device = enable; }
