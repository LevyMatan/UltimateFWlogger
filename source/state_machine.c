...
void main_state_machine(int event) {
  ...
  // Main state machine
  FW_LOG_INFO("Entered Main state machine at state: %s with event: %s", ENUM_STRING(current_state),
              ENUM_STRING_FROM_TYPE(event, "main_state_machine_events"));
...
void enqueue_event(state_machine_event_queue_t *event_queue, state_machine_event_t event) {
  ...
  FW_LOG_INFO("Enqueuing event: %s for state_machine: %s",
              ENUM_STRING_FROM_TYPE(event.event, "main_state_machine_events"),
              ENUM_STRING(event.type));
  ...
  if (event_queue->count + 1 >= event_queue->size) {
    ...
    FW_LOG_WARN("Enqueuing event: %s for state_machine: %s",
                ENUM_STRING_FROM_TYPE(event_to_override->event, "main_state_machine_events"),
                ENUM_STRING(event_to_override->type));
...
void handle_event_queue(state_machine_event_queue_t *event_queue) {
  ...
  while (event_queue->count > 0) {
    ...
    FW_LOG_INFO("Handling event: %s for state_machine: %s",
                ENUM_STRING_FROM_TYPE(event.event, "main_state_machine_events"),
                ENUM_STRING(event.type));
